...
- 스트리밍 수집을 취소하면 전달을 멈추고 **런타임에 생성 중단을 요청한 뒤에** 엔진을 놓습니다 —
  그래서 앞 생성이 도는 중에 다음 호출이 시작되는 일이 없습니다. (런타임의 `Flow` 는 취소 시
  아무것도 하지 않아서, 이 라이브러리가 명시적으로 요청합니다.)
- **그 기다림에는 시계가 달려 있습니다.** `cancelProcess()` 는 생성이 언제 실제로 끝나는지 아무것도
  약속하지 않습니다. 런타임이 요청을 흘리면 엔진이 영영 잡히고, 뒤에 줄 선 호출이 전부 같이
  멈춥니다 — 사용자는 취소했는데 앱이 조용해지는 상태죠. 그래서 대기를
  `GenerationLimits.stopGraceMillis`(기본 5초)로 제한하고, 걸린 시간을 매번
  `GenerationStats.stopWaitMillis` 로 알려줍니다. 그 시간을 넘기면 엔진을 **격리**합니다 — 해제하지
...
- `engine.close()` 는 되돌릴 수 없습니다. 이후 **생성하거나 모델을 바꾸는** 호출은 모두
  `EngineClosedException` 입니다. 정리 경로는 일부러 조용합니다 — `close(chat)` 은 경고만 남기고
  `warmUp()` 은 그냥 돌아옵니다. 이미 사라지는 중인 화면에서 불리는 것들이라서요.
...
`streamMode` 만은 추측하면 안 됩니다. 조각은 **델타**로 오거나 **누적 스냅샷**으로 오는데,
**글자만 봐서는 구분할 수 없습니다** — 모델이 같은 토큰을 되풀이하면 같은 조각이 두 번 오고,
그건 앞의 것을 그대로 담은 스냅샷과 똑같이 생겼습니다. 기본값(`StreamMode.DELTA`)은 LiteRT-LM
0.13.x 의 동작이고, 다른 런타임이면 명시해야 합니다. 안 그러면 되풀이된 글자가 조용히 사라집니다.
